...
export default function MonitoringFeed({ isMobile }: { isMobile?: boolean }) {
  ...
  return (
    <div className={`flex flex-col font-sans overflow-hidden ${
      ...
      {/* Header */}
      <div className={`flex items-center justify-between ${isMobile ? 'px-4 py-4' : 'p-5'} border-b border-white/5 bg-black/20`}>
        <h2 className={`text-lg font-bold uppercase ${isMobile ? 'tracking-widest' : 'tracking-wider'} text-white flex items-center gap-3`}>
      ...
      {/* Search and Filters */}
      <div className={`${isMobile ? 'px-4 py-3' : 'p-4'} border-b border-white/5 space-y-3 bg-black/10`}>
        <div className="relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 w-4 h-4" />
          <input type="text" placeholder="Пошук повідомлень..." 
                 className={`w-full bg-white/5 text-white ${isMobile ? 'text-xs pl-10 pr-4 py-3' : 'text-sm pl-11 pr-4 py-3'} rounded-xl outline-none border border-white/5 focus:border-red-500/50 focus:bg-white/10 transition-all placeholder:text-gray-600 font-medium`} />
        </div>
        ...
        <AnimatePresence>
          {messages.map((msg, index) => (
            <motion.div 
              ...
              className={`bg-white/5 hover:bg-white/10 rounded-2xl transition-all cursor-pointer border border-white/5 hover:border-white/10 group ${isMobile ? 'p-4' : 'p-4'}`}
            >
