...
function detectThreatType(text: string): ParsedThreat['type'] | null {
  ...
  if (t.match(/(відбій|чисто|немає загроз|спокійно|зник|поки чисто|не фіксується|локаційно втрачено|загроза застосування|можливі пуски|реб|рєб|фальш|імітаці|тренуван|загроза балістик|загроза удар|загроза артобстріл|загроза циркон|загроза кинджал|загроза ракет|загроза каб|загроза шахед|загроза бпла|імовірно|можливо)/)) return 'INFO';
