...
function detectThreatType(text: string): ParsedThreat['type'] | null {
  ...
  if (t.match(/\b(збит[оіа]|знищен[оіа]|перехоплен[оіа]|мінус|відбит[оіа]|ліквідован[оіа]|ппо\s*спрацюва|зенітн)\b/)) {
    if (!t.match(/(за добу|за ніч|за тиждень|підсумки|загалом|втрати)/)) return 'PPO';
  }
  if (t.match(/\b(циркон|3м22)\b/)) return 'ZIRCON';
  if (t.match(/\b(шахед|шахід|shahed|бпла|мопед|безпілотник|геран|гербер|дрон|ударний\s*безпілотн)\b/)) return 'DRONE';
  if (t.match(/\b(балістик|кинджал|іскандер|с-300|с-400|кн-23|кн-24|точка-у)\b/)) return 'BALLISTIC_MISSILE';
  if (t.match(/\b(х-101|х-555|х-55|х-59|х-69|х-35|калібр|кх-|3м14|крилат[аіи]\s*ракет|ракетоносц|ракетонос)\b/)) return 'CRUISE_MISSILE';
  if (t.match(/\b(ракет[аиоу]|пуск\s*ракет|ракетн)\b/)) {
    if (t.match(/(повітрі|летить|рух|курс|напрямок)/)) return 'CRUISE_MISSILE';
    return 'MISSILE';
  }
  if (t.match(/\b(ту-95|ту-160|ту-22|ту22|міг-31|міг31|авіаці|су-34|су-35|су-27|су-24|су-25|бортів|стратегічн|тактичн\s*авіаці|бомбардувальник|винищувач)\b/)) return 'AIRCRAFT';
  if (t.match(/\b(каб|фаб|уаб|авіабомб|кероване?\s*бомб|планер)\b/)) return 'KAB';
  return null;
}
...
export function parseTelegramText(text: string): ParsedThreat[] {
  ...
  if (lowerText.match(/\b(за добу|за ніч|за тиждень|підсумки|статистика|загинул[оіа]|постраждал|загалом\s+за|втрати\s+рф|зведення\s+за|наслідки)\b/)) {
    return [{ type: 'SUMMARY', lat: null, lng: null, confidence: 100, direction: null }];
  }

  // Filter out "all clear"
  if (lowerText.match(/\b(відбій|чисто\s+по|наразі\s+чисто|немає\s+загроз|спокійно|не\s+зафіксован|загроз\s+немає|поки\s+чисто)\b/)) {
  ...
  const hasAction = lowerText.match(/\b(летить|летят|рух|зліт|пуск|запуск|напрямок|курс|фіксує|повітрі|пускові|вибух|атак|йде|небезпека|відмічено|збито|знищено|мінус|перехоплено|відбито|тривога|загроза|увага|старт|виявлен|входить|маршрут|виліт|наближ|обережно|слідкуйте|попереджен)\b/);
  ...
  for (const line of lines) {
    ...
    if (lineLat !== null && lineLng !== null) {
      const hasLineMention = lineLower.match(/(бпла|шахед|ракет|балістик|калібр|дрон|крилат|каб|фаб|х-101|х-55|циркон|курс|напрямок|збито|мінус|летить|рух|пуск)/);
      if (hasLineMention) {
        results.push({
          type: lineType,
          lat: lineLat + jitter(),
