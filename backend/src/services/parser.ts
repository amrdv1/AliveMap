...
export function parseTelegramText(text: string): ParsedThreat[] {
  ...
  const hasAction = lowerText.match(/(летить|летят|рух|зліт|пуск|запуск|напрямок|курс|фіксує|повітрі|пускові|вибух|атак|йде|небезпека|відмічено|збито|знищено|мінус|перехоплено|відбито|тривога|загроза|увага|старт|виявлен|входить|маршрут|виліт|наближ|обережно|слідкуйте|попереджен)/);
