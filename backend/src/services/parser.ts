...
function detectThreatType(text: string): ParsedThreat['type'] | null {
  ...
  if (t.match(/(ціль|загроз|намотує|розвернув|повернув|пролітає|залишивс|остался|в\s*бік|в\s*напрямку|між\s+|кружляє|вектор)/)) return 'UNKNOWN';
