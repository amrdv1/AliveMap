...
export const CITY_COORDS: Record<string, {lat: number, lng: number}> = {
  ...
  // ═══ ІНШІ ТА ПРИКОРДОННІ ЗОНИ ═══
  "брянщин": { lat: 53.2435, lng: 34.3634 },
  "білорусс": { lat: 51.9000, lng: 30.0000 },
  "водосховищ": { lat: 50.8250, lng: 30.4400 },
  ...
  "славут(?!ич)": { lat: 50.3000, lng: 26.8667 },
...
function detectThreatType(text: string): ParsedThreat['type'] | null {
  const t = text.toLowerCase().replace(/a/g, 'а').replace(/o/g, 'о').replace(/e/g, 'е').replace(/i/g, 'і').replace(/p/g, 'р').replace(/c/g, 'с').replace(/x/g, 'х').replace(/y/g, 'у');
  const tPadded = ' ' + t.replace(/[^а-яіїєґa-z0-9]/g, ' ') + ' ';
  
  if (t.match(/(відбій|немає загроз|загрози немає|немає загрози|не активні|тихої ночі|без цілей|спокійно|поки чисто|не фіксується|локаційно втрачено|загроза застосування|можливі пуски|фальш|імітаці|тренуван|загроза балістик|загроза удар|загроза артобстріл|загроза циркон|загроза кинджал|загроза ракет|загроза каб|загроза шахед|загроза бпла)/) || tPadded.match(/ (реб|рєб|чисто|зник) /)) return 'INFO';
  ...
  if (t.match(/(збито|знищено|мінус|ppo|ппо|робота ппо|вибух|відпрацю|збиття|гучно|не фіксує|не фиксиру|впав|відбилися)/) || tPadded.match(/ (зник|чисто|локаційно втрачено) /)) {
...
export function parseTelegramText(text: string): ParsedThreat[] {
  let lowerText = text.toLowerCase().replace(/a/g, 'а').replace(/o/g, 'о').replace(/e/g, 'е').replace(/i/g, 'і').replace(/p/g, 'р').replace(/c/g, 'с').replace(/x/g, 'х').replace(/y/g, 'у');
