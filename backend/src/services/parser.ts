...
function detectThreatType(text: string): ParsedThreat['type'] | null {
  ...
  if (t.match(/(відбій|немає загроз|спокійно|поки чисто|не фіксується|локаційно втрачено|загроза застосування|можливі пуски|\bреб\b|\bрєб\b|фальш|імітаці|тренуван|загроза балістик|загроза удар|загроза артобстріл|загроза циркон|загроза кинджал|загроза ракет|загроза каб|загроза шахед|загроза бпла|\bчисто\b|\bзник\b)/)) return 'INFO';
